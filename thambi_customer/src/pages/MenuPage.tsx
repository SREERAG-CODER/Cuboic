...
export function MenuPage() {
    ...
    useEffect(() => {
        ...
        try {
            const o = localStorage.getItem('thambi_active_orders');
            if (o) {
                ...
                if (validOrders.length !== parsed.length) {
                    if (validOrders.length === 0) {
                        localStorage.removeItem('thambi_active_orders');
                    } else {
                        localStorage.setItem('thambi_active_orders', JSON.stringify(validOrders));
                    }
                }
            }
        } catch (e) {
